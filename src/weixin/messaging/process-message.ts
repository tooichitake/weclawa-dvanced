...
export async function processOneMessage(
  ...
  logger.info(
    `route-resolved: accountId=${deps.accountId} from=${finalized.From} to=${finalized.To} agentId=${route.agentId ?? "none"} sessionKey=${redactToken(route.sessionKey)} mainSessionKey=${redactToken(route.mainSessionKey)} hasContextToken=${Boolean(contextToken)}`,
  );
